...
HELP_TEXT = (
    ...
    "*Chat with Claude*\n"
    "Ask questions, brainstorm, summarize, compare options, or work from source material you paste or upload. "
    "Replies stream into the same message as they arrive, so you see the answer building in real time.\n\n"
    "*Generate and host a page*\n"
    "Describe the page you want and I will create a self-contained HTML page, save it to the tailnet, and reply with a URL. "
    "While I work, the placeholder message updates with elapsed-time ticks like `Generating market-map.html — 15s elapsed...`. "
    "If the request is too thin, I will ask one clarification question first and use your next DM as the answer.\n"
    "Examples:\n"
    ...
    "- `generate market-map.html` (I will ask you what to put on it)\n\n"
    ...
    "Upload a single self-contained `.jsx` React component with no imports and I will publish both the source and a runnable HTML page that loads React from a CDN.\n\n"
    "*Revise published pages in place*\n"
    "After I publish a page, reply with natural revision instructions and I will update the same URL while saving the old version privately for rollback.\n"
    "Examples:\n"
    ...
    "- `rollback` restores the previous version of the most recent page.\n"
    "- `rollback market-map.html` restores the previous version of a specific page.\n"
    "- `history` lists your recent pages with version counts and timestamps. Old versions are kept privately and are not served on the tailnet.\n\n"
    "*Pending clarifications*\n"
    "If I have asked a clarification question, your next DM is treated as the answer. Send `cancel` to clear a pending question without generating anything.\n\n"
    "*Filenames, access, and cleanup*\n"
    ...
    "Pages that are older than the configured TTL (default 90 days) are cleaned up automatically on startup and hourly while the bot runs.\n\n"
    "*Command reference*\n"
    "- `help` / `guide` / `usage` - show this message\n"
    "- `cancel` - clear a pending clarification\n"
    "- `generate <filename>.html <prompt>` - explicit generation with a chosen filename\n"
    "- `revise <filename>.html <instructions>` - revise a specific published page\n"
    "- `rollback [filename.html]` - restore the previous version\n"
    "- `history` - list your recent pages"
)
